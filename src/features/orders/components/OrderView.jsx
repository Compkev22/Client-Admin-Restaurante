...
export const OrderPage = () => {
  ...
  const filteredOrders = activeTab === "Todos" ? orders : orders.filter(o => o.estado === activeTab);
  ...
  return (
    <div className="space-y-8 animate-fadeIn p-6">
      ...
      <OrderGrid 
        ...
        onPay={(order) => { setSelectedOrderToPay(order); setIsPaymentOpen(true); }}
        onDetail={(order) => { setSelectedOrderDetails(order); setIsDetailOpen(true); }} 
      />
      {/* Pasamos fetchOrders para que la lista se actualice al cerrar los modales */}
      <OrderModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); fetchOrders(); }} />
      <PaymentWizardModal isOpen={isPaymentOpen} onClose={() => { setIsPaymentOpen(false); fetchOrders(); }} orderData={selectedOrderToPay} />
