    ...
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 hover:shadow-xl transition-all flex flex-col relative group">
      <div className="flex justify-between items-start mb-4">
        ...
        <span className="text-sm font-bold text-gray-400">
          <img src={orderTimeIcon} alt="Hora" className="w-5 h-5 inline-block mr-2" />
          {order.horaPedido}
        </span>
      </div>

      <div className="mb-6">
        <h3 className="text-2xl font-black text-gray-800 uppercase italic leading-none mb-1">ORD-{order._id.slice(-4).toUpperCase()}</h3>
        <p className="text-sm text-gray-500 font-bold">{order.orderType === 'DINE_IN' ? order.mesaId : 'Cliente Mostrador/App'} • {order.items} items</p>
      </div>
      ...
      <div className="flex justify-between items-end mb-6">
        ...
        <span className={`text-[10px] font-black uppercase px-3 py-1 rounded-full border ${getStatusColor(order.estado)}`}>{order.estado}</span>
      </div>
      ...
      <button 
        ...
        className={`w-full py-3 rounded-xl font-black uppercase tracking-widest transition-all mt-auto ${
          isPayable ? 'bg-kinal-red text-white hover:bg-red-700 shadow-lg' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'
        }`}
      >
