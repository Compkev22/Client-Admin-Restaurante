...
export const getOrderTypeStyle = (type) => {
  switch (type) {
    case "DINE_IN":
      return { text: "En Restaurante", bg: "bg-blue-100", textCol: "text-blue-700", border: "border-blue-200" };
    case "TAKEAWAY":
      return { text: "Para Llevar", bg: "bg-orange-100", textCol: "text-orange-700", border: "border-orange-200" };
    case "DELIVERY":
      return { text: "A Domicilio", bg: "bg-purple-100", textCol: "text-purple-700", border: "border-purple-200" };
    default:
      return { text: type, bg: "bg-gray-100", textCol: "text-gray-700", border: "border-gray-200" };
  }
};
...
export const OrderCard = ({ order, onPay, onDetail, onEdit }) => {
  ...
  const isEditable = order.estado !== "Entregado" && order.estado !== "Cancelado";

  return (
    <div className="bg-white rounded-3xl p-5 md:p-6 shadow-sm border border-gray-100 hover:shadow-xl transition-all flex flex-col relative group">
      <div className="flex justify-between items-start mb-4">
        <span className={`text-[10px] font-black uppercase px-3 py-1 rounded-full border shrink-0 ${typeStyle.bg} ${typeStyle.textCol} ${typeStyle.border}`}>
          {typeStyle.text}
        </span>
        <span className="text-sm font-bold text-gray-400 flex items-center gap-1 shrink-0">
          <img src={orderTimeIcon} alt="Hora" className="w-4 h-4" />
          {order.horaPedido ||
      ...
      <div className="mb-4">
        <h3 className="text-xl md:text-2xl font-black text-gray-800 uppercase italic leading-none mb-1">
      ...
      <div className="flex justify-between items-end mb-4">
        ...
        <span className={`text-[10px] font-black uppercase px-3 py-1 rounded-full border shrink-0 ${getStatusColor(order.estado)}`}>
          {order.estado}
        </span>
      </div>
