...
export const ReservationCard = ({ res, branches, users, onEdit, onToggleStatus }) => {
  ...
  return (
    <div className={`bg-white rounded-3xl p-6 shadow-sm border-2 transition-all relative group flex flex-col ${isInactive ? "opacity-60 border-red-100" : "border-gray-50 hover:border-orange-100 hover:shadow-xl"}`}>
      <div className="absolute top-6 right-6">
        <span className={`text-[10px] font-black uppercase px-3 py-1 rounded-full border ${statusColor}`}>{res.status}</span>
      </div>
      ...
      <div className="space-y-3 mb-4">
        <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100 flex items-center justify-between">
          <div>
            ...
            <p className="font-bold text-gray-800 truncate max-w-[150px]">{clientName}</p>
          </div>
      ...
      <div className="flex justify-end gap-2 mt-auto pt-4 border-t border-gray-50">
        <button onClick={() => onEdit(res)} className="p-2.5 bg-gray-50 hover:bg-gray-100 rounded-xl transition-all text-gray-600 hover:text-kinal-orange font-black text-[10px] uppercase flex items-center gap-2">
          <img src={iconEdit} className="w-4 h-4 opacity-70" alt="Edit" /> Editar
        </button>
        <button
          onClick={() => onToggleStatus(res._id, res.clientId?.UserName)}
          className={`p-2.5 rounded-xl transition-all font-black text-[10px] uppercase flex items-center gap-2 ${isInactive ? "bg-green-50 text-green-600 hover:bg-green-100" : "bg-red-50 text-red-600 hover:bg-red-100"}`}
        >
