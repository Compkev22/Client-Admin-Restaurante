...
export const TableCard = ({ table, onEdit, onDelete }) => {
  ...
  return (
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 relative flex flex-col items-center text-center hover:shadow-lg transition-all">
      ...
      <div className="absolute top-4 right-4 flex gap-1">
        <button onClick={() => onEdit(table)} className="p-1.5 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors">
          <img src={iconEdit} className="w-4 h-4 opacity-60" alt="Editar" />
        </button>
        <button onClick={() => onDelete(table)} className="p-1.5 bg-red-50 hover:bg-red-100 rounded-lg transition-colors">
